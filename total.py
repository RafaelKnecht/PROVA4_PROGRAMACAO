...
while True:
        nome_carro = input(
            "Digite o nome do carro que deseja comprar (ou digite 'fim' para encerrar): ")
        if nome_carro.lower() == "fim":
            break

        preco_carro = float(
            input("Digite o preço que está disposto a pagar pelo carro: "))

        print(f"O usuário gostaria de saber se o carro {
              nome_carro} está disponível e gostaria de pagar {preco_carro} reais nesse carro.")
        ...
        if resultado_procura == "Carro encontrado!":
            ...
            print(f"O usuário gostaria de um carro {nome_carro} na qualidade de {qualidade_carro}. O valor do {
                  nome_carro} é o mesmo recebido na entrada. Já o valor da {qualidade_carro} é recebido através da avaliação do preço.")
